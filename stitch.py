...
for z in [1, 4, 5]:
    for i, folder in enumerate(glob(f'raw/z{z}/*')):
        print(f'stitch {i+1}/{len(glob(f"raw/z{z}/*"))}')
        ...
        if restitch_existing or not path.exists(stitched_file_name):
            ...
            if z == 1:
                if len(stitches) == 2:
                    merge_images_z1(stitches).save(stitched_file_name, compress_level=8)
            elif z == 4:
                y = int(path.split(files[-1])[1][4:6])
                x = int(path.split(files[-1])[1][8:10])
                if len(stitches)  == (x+1)*(y-1):
                    merge_images_z4(stitches, x+1, y-1).save(stitched_file_name, compress_level=8)
            else:
                if len(stitches) == 96:
                    merge_images_z5(stitches).save(stitched_file_name, compress_level=8)